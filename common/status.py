...
REASON = {
    ...
    'exhausted-27': 'the cuboctahedron pins the twelve directions, four zero-sum triangles pairwise at '
                    ...
                    'in an hour, four MUTUALLY ORTHOGONAL ones give 1334 and sequential extraction '
                    'with side 0 at its exact optimum 762 gives 1829: the four sides compete, and 4 x 762 '
                    'is a bound that forgets the cross-side threshold, not room.  The eight zero-sum triples give nothing over '
                    'the four of a partition: shares-a-direction is the cube graph Q3 on them, its '
                    'complement is 4-chromatic, so every eight-triple assignment maps into a '
                    'four-side one.  What was NOT exhausted is the SECOND layer -- a maximum '
                    '3-colourable induced subgraph on 3110 admissible owners, one condition on a line '
                    'and none across lines.  303 are now taken, 104 + 99 + 100, over the first layer '
                    'of 2095 class heads and 164 axial normals that B. Lindow obtained by '
                    're-optimising the three item types jointly; his own second layer on it was 291, '
                    'and 284 was the best over the earlier first layer.  NO ceiling is in hand: the '
                    '297 once recorded here was three times a REALISED single-line count, which '
                    'bounds alpha from below, and 303 refutes it.  '
    ...
    'classes-31': 'the equator (196 560), the direction weight (84 = floor(2 x 126 / 3)), the AXIS '
                  ...
                  'where no sign word could separate them, never occur.  TWO of the 88 are '
                  'CERTIFIED unavoidable: the 15 codes at an octad are the points of a PG(3,2) '
                  'whose 35 lines are the 35 splits of the octad into two 4-sets, and for one '
                  'split the heavy classes it links must be 2-colourable, so an odd cycle -- a '
                  'triangle at octad 458 over classes 20, 22 and 28 -- forces a collision '
                  'costing |U n V| = 2 lines whatever the sign words.  The other 86 are search: '
                  'on octad coverage, bit budget, pair probability and code agreement alike, '
                  'five fresh builds match the shipped family and lose twice as many lines, and '
                  'a support set optimised for coverage EXACTLY over all 3795 x 2016 = 7 650 720 '
                  'of them reaches 1547 against this family\'s 1670 and realises 79 lines WORSE.  '
                  'Nor can a short class take a different line back: a 248-line class is '
                  'recovered from any 216 of its lines, so each short class is a subset of a '
                  'unique full one.  What is left is search depth '
                  '(research/collab2531/BRAINSTORM_2831.md section 9; '
                  'research/collab2531/dim31/{kappa31,reslot31,pg32}.py; '
