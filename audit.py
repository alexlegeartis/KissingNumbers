...
if '--write-results' in sys.argv and not fail:
    ...
    PKGDOC = {
        ...
        'dim25-lens-heads': '1016 heads in the lens of a minimal vector, no removal shared, plus one non-lattice equator point.  972 are class heads x = u + t- v and the other 44 are exact rationals, and the 44 are a MAXIMUM INDEPENDENT SET of the whole admissible pool rather than heads added one at a time: solving max |x| over the region of each owner against the class core alone gives a pool of 1188 heads, whose greedy answer is the 35 this package used to ship and whose CP-SAT answer is 44',
        'dim26-27-iota-triangles': 'the coset triangle of three norm-6 vectors on every triangle of directions, the side chosen per head; in 26 the second side is the involution image of the first.  On top of it a layer of FREE heads y = +-2v, which remove no equator point and are therefore worth 3 where a class head is worth 2: their conflicts with the class layer concentrate onto about 72 heads, so the deletions are shared and the trade is solved jointly -- 129 free heads for 77 class heads in dimension 27, since improved by B. Lindow to 2095 class heads and 164 axial normals by optimising class heads, axial normals and the second layer TOGETHER rather than in sequence.  In dimension 26 the free heads are given up instead: they are the ONLY thing blocking its second layer, because a free head y = 2v makes <Y,u> = 2<v,u> a multiple of 16, hence 32 the moment <v,u> = 16, above the bar of 24 whatever the geometry.  Over the class heads alone 1188 owners qualify and 324 fit, so dropping all 96 free heads costs 288 and buys 324: +36, and no free head can be put back.  Both dimensions carry a SECOND cap layer at |x|^2 = 3 with |y| = 1, at |x|^2 = 3: such a cap points anywhere (|y||a| = 2), carries the antipodal pair +-y, and the scaled owner sqrt(3)/2 u deletes exactly u, so it is +1 each.  Its direction must clear the twelve first-layer directions, and the cuboctahedron covers at 45 degrees where the hexagon covers at 30, so the bar is <Y,u> <= 32 against 24 -- 3009 admissible owners against 1188, and 284 heads on the three square-face lines against 324 on the three vertex lines of the hexagon.  That layer is a maximum 3-COLOURABLE induced subgraph -- one condition on a line, none across lines.  In dimension 27 it now carries 299 heads (103 + 98 + 98) over 3110 admissible owners.  Beware the ceiling that was once recorded for it: three times a REALISED single-line count is not an upper bound at all, and 299 exceeds both figures (285, 297) that were derived that way',
