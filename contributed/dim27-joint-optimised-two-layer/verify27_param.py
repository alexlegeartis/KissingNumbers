# -*- coding: utf-8 -*-
# COPY of verifications/improved/dim26-27-iota-triangles/verify27.py (A. Kravatskiy, MIT licence, see the
# LICENSE file at the repository root), with ONE line changed: the final check pinned the first-layer total
# of the configuration shipped in that package (201225); here the expected first-layer total is the first
# command-line argument, defaulting to 201225. Reason: the pin is the only line on which the original fails
# for a different configuration. Everything else, including lib/, is byte-identical to the original.
"""Exact verification of  K(27) >= 201509   (python verify27.py)
...
chk('the first layer alone gives %s' % (sys.argv[1] if len(sys.argv) > 1 else 201225), first == int(sys.argv[1] if len(sys.argv) > 1 else 201225), '%d' % first)
if bad:
