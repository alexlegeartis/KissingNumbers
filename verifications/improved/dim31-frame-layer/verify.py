...
so if every owner is TYPE B for the frame -- |<u,v_i>| <= 2 for all 24 heads, which for the
frame 8e_i means the owner has shape (+-2^8, 0^16), the bound being attained on the eight
coordinates of its octad and 0 on the other sixteen -- the threshold on the directions is
...
req(np.abs(OWN).max() == 2,
    "every owner is TYPE B for this frame: |<u, 8e_i>|/8 is 0 or 2, never 3 or 4 -- which with "
    "norm 32 forces the shape (+-2^8, 0^16)")
