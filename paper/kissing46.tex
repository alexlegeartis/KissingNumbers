...
\usepackage[hidelinks,
  pdftitle={New lower bounds for kissing numbers in dimensions 18 through 96},
...
\title[Kissing numbers in dimensions 18 to 96]
      {New lower bounds for kissing numbers\\in dimensions 18 through 96}
...
% values.tex is GENERATED from RESULTS.md by mkvalues.py: every claimed bound, once.
\input{values}
\begin{document}

\begin{abstract}
The kissing number $K(n)$ is the largest number of unit vectors in $\R^n$ with
pairwise inner products at most $\tfrac12$. We improve the best known lower bound
in $53$ dimensions between $18$ and $96$.

All but five come from the extremal even unimodular lattices $\Leech$, $\Pff$ and
$\Gsz$, by two constructions. For cross-sections we apply Venkov's theorem --- the
minimal shell of such a lattice is a spherical $11$-design --- to monomials in the
inner products with several fixed minimal vectors, which bounds the number of
minimal vectors orthogonal to all of them; each bound carries an exact rational
dual certificate, and in dimensions $46$ and $47$ the same programme returns the
published values exactly, which validates it. The layered construction puts the
minimal shell on an equator of $\R^{n}\oplus\R^{k}$ and adjoins lifted copies of
part of it. Its level, and the height and norm of its head, are all free
parameters: at height $\sqrt2$ a norm-$8$ head lets a whole frame of $\Leech$
carry the whole cross-polytope of $\R^{k}$, which is $96k$ points that delete
nothing. Every dimension from $25$ to $31$ is improved --- $K(25)\ge\Kh{25}$,
$K(26)\ge\Kh{26}$, $K(27)\ge\Kh{27}$, $K(28)\ge\Kh{28}$, $K(29)\ge\Kh{29}$,
$K(30)\ge\Kh{30}$, $K(31)\ge\Kh{31}$ --- as are $K(38)\ge\Kh{38}$ and the
dimensions $49$--$61$, $68$--$71$ and $73$--$95$.

Of the other five, four are the chain of Edel--Rains--Sloane evaluated against
the current code tables: $K(39)\ge\Kh{39}$, $K(62)\ge\Kh{62}$,
$K(63)\ge\Kh{63}$ and, overtaking $\Gsz$ at $n=96$, $K(96)\ge\Kh{96}$. The
fifth is dimension $18$, where a hexagon of tiers
over the odd Barnes--Wall lattice has three six-set families that may be chosen as
bent cosets of $\mathrm{RM}(1,4)$; this raises its outer tier from $256$ vectors
to $960$ and gives $K(18)\ge\Kh{18}$, against $\Kp{18}$.

Every bound is certified in exact arithmetic by a public verification package; no
floating-point computation decides any of them.
\end{abstract}
...
the fifty-three improvements below lie in these ranges.
...
\begin{table}[ht]
\caption{The fifty-three improvements, grouped by construction; $\#$ is the number
of dimensions in a row. The gain is $K_{\mathrm{here}}-K_{\mathrm{prev}}$ and the
factor their ratio, computed dimension by dimension, with the factor
attaining both its extremes at an end of the dimension range, to the two decimals
shown; the largest gain need not, and in the $28$--$31$ row it is dimension $30$. ``Previously'' is \cite{cohn-table}
...
\midrule
$\Gsz$ cross-sections & $68$--$71$ & $4$ & $\Kgain{71}$ & $1.09$--$7.85$ & \S\ref{ss:g72}\\
layers over $\Leech$ & $25$--$27$, $38$ & $4$ & $\Kgain{38}$ & $1.00$--$1.04$ & \S\ref{ss:d25}--\ref{ss:d38}\\
second layers over $\Leech$ & $28$--$31$ & $4$ & $\Kgain{30}$ & $1.00$--$1.00$ & \S\ref{ss:d293031}\\
layers over $\Pff$ & $49$--$61$ & $13$ & $\Kgain{61}$ & $1.00$--$1.14$ & \S\ref{ss:big}\\
the \textsc{ers} chain & $62$, $63$ & $2$ & $\Kgain{63}$ & $1.36$--$2.64$ & \S\ref{ss:d6263}\\
the \textsc{ers} chain at $96$ & $96$ & $1$ & $\Kgain{96}$ & $2.07$--$2.07$ & \S\ref{ss:d96}\\
layers over $\Gsz$ & $73$--$95$ & $23$ & $\Kgain{95}$ & $1.00$--$1.04$ & \S\ref{ss:big}\\
constant-weight codes & $39$ & $1$ & $\Kgain{39}$ & $1.00$--$1.00$ & \S\ref{ss:d39}\\
the bent-coset hexagon & $18$ & $1$ & $\Kgain{18}$ & $\Kfac{18}$--$\Kfac{18}$ & \S\ref{sec:d18}\\
\midrule
total & & $53$ & & &\\
\bottomrule
\end{tabular}
...
$2026$. The rows
with factors near $1$ span the most dimensions, $46$ of the $53$, though the two
...
\emph{The fifty-three listed below.} We have found no earlier published claim in dimensions
$18$, $25$--$31$, $38$, $39$, $49$--$63$, $68$--$71$ or $73$--$96$. The one-point
distribution of the minimal shell of $\Gsz$ follows from Venkov's theorem in a
few lines; the bound in dimension $71$, which is that distribution's $n[0]$, may therefore
already be known to specialists, though we have found no record of it. Dimensions $62$, $63$ and $96$
...
are treated in \S\ref{sec:ers}, and dimension $18$, which uses neither lattice and
is treated in \S\ref{sec:d18}. \S\ref{sec:results} collects the full table.
...
dimensions in which the linear programming bound already determines the kissing
number. No such form is available
...
dimensions $25$ through $31$ in the published table. Their configurations are the
input to the template theorems of \S\ref{ss:d25} and \S\ref{ss:d27}, which
construct no new spherical code; the bounds of this paper in dimensions $25$ to
$27$ come from the head configurations of \S\ref{ss:d25} and \S\ref{ss:d2627}. Takhanov, Assylbekov and Yun
...
$1.98\times10^5$ and $1.29\times10^{10}$ points, apart from dimension $18$'s $8\,358$. A lattice together with a few
integer parameters specifies each of them, apart from the four code-theoretic
ones, which a chain of codes specifies. They lie beyond the reach of a
coordinate search. We obtain them from the structure of three extremal lattices,
apart from dimension $18$ (\S\ref{sec:d18}), which uses the odd Barnes--Wall lattice.

Forty-three of the fifty-three dimensions below lie in ranges in which the published
...
Below dimension $25$ the methods of this paper yield only dimension $18$
(\S\ref{sec:d18}). Every published record in dimensions $9$ through $19$ is
maximal, in the sense that no further point may be added to it, as are the
cross-sections $\Lambda_{21}$, $\Lambda_{22}$ and $\Lambda_{23}$; maximality
forbids extending a configuration, not replacing it, and dimension $18$ is
replaced. The constant-weight-code construction of \cite{cohn-li2024} is at its
exact ceiling in dimensions $18$, $20$ and $21$, and dimension $19$ is closed at
the value of Ho (\S\ref{ss:obstructions}) --- but in dimension $18$ that is not the
construction holding the record, and the one that does is improved here.
...
Let $t$ be the strength of the spherical design $\{\hat v:v\in\Lambda\}$ of
Proposition~\ref{prop:venkov}, so $t=11$ when $24\mid n$ and $t=7$ for $E_8$. For
every exponent vector $p=(p_1,\dots,p_k)$ of non-negative integers with
$\sum p_i\le t$ and $\sum p_i$ even,
...
\]
A cell violating the inequality for some such $w$ is empty, unless it is the
cell of $\pm w$ itself, which then holds exactly one vector.
\end{lemma}
...
pair, $u_1-u_2$ is itself minimal, which forces $|c_1-c_2|\le\mu/2$ outside the two
cells of $\pm(u_1-u_2)$ and so excludes ten of the twelve cells that
semidefiniteness permits. Those ten cells are the zeros of
...
indexed by cells. Set $h_c=1$ when $\det G(c)=0$ and $h_c=+\infty$ otherwise. A cell with
$\det G(c)<0$ is the profile of no real vector at all and is forced to zero, and a
cell that is the profile of a minimal lattice vector of
$\mathrm{span}(u_1,\dots,u_k)$ is forced to one.
...
$627\,822\,180$ at $\det G=256$ through $527\,597\,644$ at $352$ to
$149\,415\,783$ at $512$. At a fixed determinant the
...
\]
and no bound depending only on the Gram matrix can be exact. In the notation of
...
$K(71)\ge\Kh{71}$ and $K(70)\ge\Kh{70}$. The first is the
...
$K(69)\ge\Kh{69}$ and $K(68)\ge\Kh{68}$.
\end{theorem}
...
programme's optimum does not. Of the $104$ determinant-$1024$ Gram matrices
realised by quadruples of those $24$ minimal vectors, ninety-eight yield a repaired
...
constant-weight code size it has no claim to at those lengths, the chain gives
...
of \cite{boyvalenkov-cherkashin2025,ozeki2016,sun-wang2026}. The orthogonal Gram matrices over $\Leech$ at $k=3$ and
$k=4$ admit embeddings with different counts: at $k=3$ both are given, at $k=4$
($\dagger$) one of several.}
\label{tab:validate}
\begin{tabular}{llrrr}
\toprule
...
and $3D_3$ carry one and the same Gram matrix --- $A_3$ and $D_3$ are the same
abstract lattice, so Sun--Wang's two dimension-$45$ complements have a single Gram
matrix between them --- so the programme returns one interval for both, yet their
sizes differ:
...
\]
are both pairwise orthogonal with every norm equal to $32$, the minimum in these
coordinates, so they share one Gram matrix $32I_3$, and their cross-sections have $19\,530$ and $19\,962$ elements ---
...
\S\ref{ss:d27} show that the additional points are available. The column $k=2$
has no slack in this family; \S\ref{ss:d2627} leaves the family, and dimension
$26$ moves with it. Nor have the columns $k=4$ to $k=7$ any slack;
\S\ref{ss:d293031} adds a fourth family at a different height, and dimensions
$28$ through $31$ move with that. Every column of the table is therefore beaten,
though four of them only by leaving the family the table describes.
...
The gain is $+2$ over any independent set used in its place. The level argument
...
and B.~Lindow the caps are rebuilt from \emph{heads}: points $x$ of squared
length $3$ (norm-$4$ units) lying in the lens of a minimal vector, each removing
exactly its owner from the equator. Two compatible heads never share a removal,
so the count is exact: the equator loses its $H$ owners and gains the $2H$ points
$\pm x$, the two poles are admissible, and one further equator point
$-\tfrac{2}{\sqrt6}v$ that is not a lattice vector is added, giving
$196\,560-H+1+2H+2=196\,560+H+3$.

The heads come in blocks. Fix a vector $v\in\Leech$ of norm $6$, take the $552$
minimal vectors $z$ with $\ip{z}{v}=-3$, and push each of them towards $v$ by
$t_-=(3-\sqrt3)/6$. Every pair of heads within one block then meets the bound
$\ip{x}{x'}=1$ exactly, so a block is a tight packing. Its size is optimal: a
block projects to a code of pairwise cosine $\tfrac15$ in $\R^{23}$, and the
largest such code has $552$ points. Searched over the whole norm-$6$
shell of $16\,773\,120$ vectors, a union of blocks reaches $971$ heads --- three
vectors $v$ with pairwise inner products $-4$, $0$ and $-3$ --- and a fourth
vector contributes exactly one head more. Every remaining minimal vector that
can own a head over that core contributes one candidate, the point of largest
norm in its own polytope. That pool is complete at $1\,188$ heads over
$1\,188$ distinct owners. The supplement is an independent set in the conflict
graph the pool carries, found by maximising over that graph rather than by
growing the set head by head. The shipped configuration takes $45$ of them, so
$H=1\,016$ and
$K(25)\ge\Kh{25}$, verified in exact arithmetic by the package
\texttt{dim25-lens-heads}; its ceiling is in \S\ref{ss:obstructions}. The level
argument above remains the template's own bound and is kept as such.
...
B.~Lindow found the same construction independently (private communication
through H.~Cohn, September 2026).
...
\emph{Superseded, 2026-09-08.} The dimension-$27$ entry of the repository no
longer rests on Theorem~\ref{thm:27}; see \S\ref{ss:d2627}. The theorem and
the partition argument remain the template's own bound, and the twelve
directions they fix are the ones the new configuration uses.

\subsection{Dimensions 26 and 27: the coset triangle on every triangle of
directions}\label{ss:d2627}

\begin{theorem}\label{thm:2627}
$K(26)\ge\Kh{26}$ and $K(27)\ge\Kh{27}$.
\end{theorem}

Both are joint work in progress with H.~Cohn and B.~Lindow, and the account is
the separate note of \S\ref{ss:d25}; we record the shape and the verification.
Write a point of $\R^{24+k}$ as $(x,y)$ with $y\in\R^{k}$, normalise every point
to squared length $4$ so that compatibility is $\ip{P}{Q}\le2$, and put the
$K(k)$ axis points $(0,a)$, $|a|=2$, on a hexagon ($k=2$) or a cuboctahedron
($k=3$). A cap point $(x,y)$ then has $|y|\le\sec\theta$ for the covering radius
$\theta$ of the axis set; for the hexagon $\theta=30^{\circ}$ forces $|x|^{2}\ge8/3$
with equality forcing $y$ onto the six edge midpoints, a head at $|x|^{2}=8/3$
carries a zero-sum triangle of directions, and two cap points there are
compatible iff $\ip{x}{x'}\le2-\tfrac43\cos\angle(y,y')$: $\tfrac23$ on a common
direction, $\tfrac43$ at $60^{\circ}$, no condition at $120^{\circ}$ or $180^{\circ}$.
So the layer is one head set per triangle of directions --- two for the hexagon,
four for the cuboctahedron, whose twelve vertices are four zero-sum triangles
every two of which have a pair at $60^{\circ}$ --- each a code of cosine at most
$\tfrac14$ on the sphere $|x|^{2}=8/3$, cross-constrained at cosine $\tfrac12$, and
the count is $196\,560+K(k)+2(P-F)+3F$ for $P$ heads of which $F$ remove nothing.

Every head is $y/3$ with $y=3u+v$ integral, $u$ a minimal vector (the owner,
which the head removes) and $v$ a Leech vector of norm $6$ with $\ip{u}{v}=-3$,
or a free head $y=\pm2v$; a head removes exactly its owner, and two heads on one
owner have $\ip{y}{y'}\ge16$, above every threshold, so removals are not shared.
Three such $v$ with $v_{1}+v_{2}+v_{3}=0$ carry $1656$ heads over three
disjoint blocks $\Sigma(v_{i})=\{u:\ip{u}{v_{i}}=-3\}$ of $552$, and on one side
the exact optimum is $762$ of them, against $552$ for a single $v$, which is
Cohn's construction. In dimension $26$ the same triangle serves both sides: with
$\iota(u)=-v_{i}-u$, a fixed-point-free involution of each block,
$\ip{3u+v_{i}}{3\iota(u')+v_{i}}=15-9\ip{u}{u'}$, which respects the cross
threshold $12$ exactly when $\ip{u}{u'}\ge1$; so a side $S$ and its image
$\iota(S)$ fit on the two triangles of the hexagon whenever the owners of $S$ are
pairwise at inner product $1$ or $2$ inside each block, and an optimal $S$ with
that property exists. The shipped configuration, found by a solve in which each
head chooses its side, has $1458$ class heads, $486$ over each lean of the
triangle $v_1,v_2,v_3$; its $96$ free heads are given up for the second cap layer described
below, which they alone were blocking, and
$196\,560+6+2\cdot1458+324=\Kh{26}$.  A free head is worth $3$ against a
class head's $2$ and removes nothing, so the free layer is bought by deleting the
class heads that conflict with it; what makes that pay is that the conflicts
concentrate, about $72$ class heads being the blockers for the whole norm-$6$
shell and $489$ candidates sharing one of them, so the two choices have to be made
together, as $\max 3\lvert F\rvert-\sum_{h}\mathrm{cost}(h)$ over the deleted
heads $h$.  Priced one candidate at a time every free head looks unaffordable. In dimension $27$ the axis is
the cuboctahedron rotated by $45^{\circ}$ about a coordinate axis, whose largest
cosine to a direction is $(2+\sqrt2)/4$ and whose inner product with a cap point
is at most $(2+\sqrt2)/\sqrt3<2$; two triangles of leans with cross Gram matrix
the circulant of $(3,0,-3)$ carry the class heads over the four sides, alongside
free heads that remove nothing. The three kinds of point are priced differently --- a class head is worth
$+2$, a free head $+3$, a second-layer head $+1$ --- and they compete for the same
owners, so they have to be chosen \emph{together}. Choosing them in sequence, as
the first version of this construction did, gives $2133$ class heads, $129$ free
heads and $284$ second-layer heads. Choosing all three at once, which is
B.~Lindow's contribution, gives $2095$, $164$ and $291$: $38$ class heads
are surrendered, at $-76$, to buy $35$ free heads at $+105$ and $7$
second-layer heads at $+7$, a net $+36$. The trade that pays is one the sequential
search had already priced and declined, because in isolation it loses.  Both dimensions carry a second cap layer, at $|x|^{2}=3$ with $|y|=1$;
what separates them is not whether it exists but what it costs.  Such a cap satisfies
$\ip{y}{a}\le2$ whatever direction it points, since $|y||a|=2$; it carries the antipodal
pair $\pm y$ and no more, three directions needing pairwise cosine $\le-1$; and the scaled
owner $x=\tfrac{\sqrt3}{2}u$ deletes exactly $u$, because $\ip{x}{z}>2$ forces
$\ip{u}{z}>4/\sqrt3$, that is $\ip{u}{z}=4$.  Two cap points for one deletion: $+1$ each.
Its direction must clear the first-layer directions, whose covering radii differ ---
$30^{\circ}$ for the hexagon, $45^{\circ}$ for the cuboctahedron --- so a first-layer head
may meet a second-layer owner at $\ip{Y}{u}\le24$ in dimension $26$ and at
$\ip{Y}{u}\le32$ in dimension $27$.  A free head $y=\pm2v$ has $\ip{Y}{u}=2\ip{v}{u}$, a
multiple of $16$: it clears the bar $32$ and can never clear the bar $24$.  Dimension $27$
therefore keeps its free heads and its second layer at once, $303$ heads over $3\,110$
admissible owners, while dimension $26$ can open the layer only by surrendering every free
head --- which is what the count above does, $96$ free heads at $3$ apiece against $324$
second-layer heads at $1$, a net $+36$.  That layer is optimal and not merely the best found:
its $1\,188$ admissible owners carry a graph under $\ip{u}{u'}=16$ that is exactly
$64$-regular, constraint programming closes its independence number at $108$, and the three
lines take three disjoint such sets, so $324$ is the maximum.  Surrendering dimension $27$'s free heads in the same
way enlarges its admissible set from $3\,009$ to $4\,634$ and comes nowhere near repaying
the $387$ it costs.
Its direction must clear the twelve first-layer directions, and that is where the two
dimensions part: the hexagon's six edge midpoints leave a gap of $30^{\circ}$, at which
not one of the $196\,560$ minimal vectors qualifies, while the cuboctahedron's covering
radius is $45^{\circ}$, attained at its six square-face centres $\pm e_{1},\pm e_{2},
\pm e_{3}$ -- three antipodal lines.  One quantum, and the admissible owners go from $0$
to $3110$. Over that first layer the second layer is again an
independent-set problem, and $303$ of them fit, $104+99+100$, against the $291$ of
the joint optimisation that produced the layer; that is the last $+12$.  Everything reduces to
$\ip{Y}{u}\le32$ and $\ip{u}{u'}\le8$ on a line, since
$\tfrac{\sqrt3}{48}\cdot32+\tfrac{2}{\sqrt6}=(2+\sqrt2)/\sqrt3<2$ and
$\tfrac{3}{32}\cdot8+1=\tfrac74<2$:
$196\,560+12+2\cdot2095+3\cdot164+303=\Kh{27}$. The package
\texttt{dim26-27-iota-triangles} stores the heads as integer vectors with the
index of their triangle, rebuilds the Leech vectors from the Golay code, decides
every head--equator and head--head pair by integer comparison against the
thresholds $144-96c$ ($c$ the exact largest cosine between two triangles of
directions, in the scaling with minimum $32$) and the axis inequalities in
$\mathbb{Q}(\sqrt2,\sqrt3)$, and rejects a head moved, duplicated or displaced.

\subsection{Dimensions 28 to 31: a second layer, at a free
height}\label{ss:d293031}

\begin{theorem}\label{thm:293031}
$K(28)\ge\Kh{28}$, $K(29)\ge\Kh{29}$, $K(30)\ge\Kh{30}$ and
$K(31)\ge\Kh{31}$.
\end{theorem}

These are joint work in progress with H.~Cohn and B.~Lindow, and the account is
the separate note of \S\ref{ss:d25}; as in \S\ref{ss:d2627} we record the shape
and the verification. Table~\ref{tab:calib} is exact in the columns $k=4$
through $k=7$, so the template of \S\ref{ss:calib} has no slack in any of them:
every point beyond the published value has to come from outside
\eqref{eq:families}.

Normalise every point of $\R^{24+k}$ to squared length $4$, so that
compatibility reads $\ip{P}{Q}\le2$, and write it as $(x,y)$ with $y\in\R^{k}$.
In these units the three families of \eqref{eq:families} are the equator
$(u,0)$, the caps $(\sqrt{2/3}\,u,(2/\sqrt3)z)$ and the axis $(0,2a)$, where $u$
runs over the minimal vectors of $\Leech$, now of squared length $4$.

Proposition~\ref{prop:cap} holds every lifted point at one level, and it is
\eqref{eq:c3} --- two caps on distinct lines of one $C_i$ at a common direction
--- that forces $t\ge1/(2(1-\gamma))=\tfrac23$. That constraint exists because a
whole independent set rides each direction. A family that places only a
\emph{few} points above each direction does not meet it, and for such a family
the level is free. Write its points as $(x,hw)$ with $|w|=1$ and
$|x|^{2}+h^{2}=4$, and call $h$ the \emph{height} and $x$ the \emph{head}. Four
thresholds move with $h$:
\begin{equation}\label{eq:heights}
\begin{array}{ll}
  \text{no equator point is deleted} & \ip{x}{v}\le2 \text{ for every minimal } v,\\[2pt]
  \text{an axis point survives}      & \ip{w}{a}\le1/h,\\[2pt]
  \text{two directions of one head}  & \ip{w}{w'}\le1-2/h^{2},\\[2pt]
  \text{two heads on one direction}  & \ip{x}{x'}\le2-h^{2},
\end{array}
\end{equation}
and against a cap the condition is
\begin{equation}\label{eq:layercap}
  \sqrt{2/3}\;\ip{x}{u}\;+\;\frac{2h}{\sqrt3}\,\ip{w}{z}\;\le\;2 ,
\end{equation}
with $u$ an owner and $z$ a direction of the part carrying it. The last line of
\eqref{eq:heights} holds the number of heads on one direction to
$2/(2-|x|^{2})$ when $|x|^{2}<2$: the unit vectors $x/|x|$ are then pairwise at
cosine at most $-c$ with $c=(2-|x|^{2})/|x|^{2}$, and unit vectors so placed
number at most $1+1/c$. The first line is what makes such a layer cheap: it
costs no equator point, so the construction pays for it only in axis points and
in the classes.

Lattice facts supply the heads. If $g\in\Leech$ has $|g|^{2}=2r$ and $v$ is
minimal, then $g-v$ has squared length $2r+4-2\ip{g}{v}$, an even integer that
is $0$ or at least $4$; it is $0$ only for $g=v$, and the value
$\ip{g}{v}=r+1$ would make it $2$, which no lattice vector has. Hence
$\ip{g}{v}\le r$: at most $3$ for a head of squared length $6$ and at most $4$
for one of squared length $8$. If $|x|^{2}=1$, so that $x=u_{0}/2$ for a minimal
$u_{0}$, Cauchy--Schwarz gives $\ip{x}{v}\le2$ with equality only at
$v=u_{0}$. So $x=v/2$ with $|v|^{2}=6$, $x=f/\sqrt3$ with $|f|^{2}=6$,
$x=v/2$ with $|v|^{2}=8$ and $x=u_{0}/2$ all satisfy the first line of
\eqref{eq:heights}, at heights $h^{2}=\tfrac52$, $2$, $2$ and $3$; these are the
four that occur. The norm of the head is a second free parameter, the two
entries at $h^{2}=2$ differ only in it, and it is the norm-$8$ one at $h=\sqrt2$
that carries all four dimensions below. One consequence of \eqref{eq:layercap} is worth
recording before the cases, because it does not depend on the height: if the
direction sits exactly at the axis threshold, $\ip{w}{z}=1/h$, the second term
is $2/\sqrt3$ whatever $h$ is, and the condition collapses to
\begin{equation}\label{eq:capthr}
  \ip{x}{u}\;\le\;\sqrt6-\sqrt2\;=\;1.0352\ldots
\end{equation}

\begin{table}[ht]
\caption{The layer of Theorem~\ref{thm:293031}, in the normalisation
$|P|^{2}=4$. The head $x$ is the $\R^{24}$ part of a layer point and $w$ its
direction; $z$ is a cap direction. All four columns are one layer, the norm-$8$
frame at height $\sqrt2$: the head runs over all $48$ vectors of a Leech frame
and the direction over the whole cross-polytope of $\R^{k}$, which is $96k$
points. The axis is the maximum $60^{\circ}$ code of \cite{ma-etal2025} that
Table~\ref{tab:calib} reproduces, carried into the frame's coordinates --- and in
dimensions $30$ and $31$ rotated first, which is free and costs the layer
nothing. No layer point deletes an equator point, and the last three lines are
the whole price.}
\label{tab:layers}
...
\toprule
$k$ & $4$ & $5$ & $6$ & $7$\\
dimension & $28$ & $29$ & $30$ & $31$\\
\midrule
height $h$ & $\sqrt2$ & $\sqrt2$ & $\sqrt2$ & $\sqrt2$\\
$|x|^{2}=4-h^{2}$ & $2$ & $2$ & $2$ & $2$\\
$|$head$|^{2}$ & $8$ & $8$ & $8$ & $8$\\
head $x$ & $v/2$ & $v/2$ & $v/2$ & $v/2$\\
directions $w$ & $\pm e_{1..4}$ & $\pm e_{1..5}$ & $\pm e_{1..6}$ & $\pm e_{1..7}$\\
axis threshold $1/h$ & $1/\sqrt2$ & $1/\sqrt2$ & $1/\sqrt2$ & $1/\sqrt2$\\
largest $\ip{w}{z}$ & $1/\sqrt2$ & $1/\sqrt2$ & $1/\sqrt2$ & $1/\sqrt2$\\
directions per head & $8$ & $10$ & $12$ & $14$\\
heads per direction & $48$ & $48$ & $48$ & $48$\\
layer points & $384$ & $480$ & $576$ & $672$\\
\midrule
axis points kept & $16$ of $24$ & $32$ of $40$ & $68$ of $72$ & $118$ of $126$\\
owner lines & $8\cdot248$ & $14\cdot248$ & $24\cdot248$ & $10\,328$\\
net gain & $\Kgain{28}$ & $\Kgain{29}$ & $\Kgain{30}$ & $\Kgain{31}$\\
\bottomrule
\end{tabular}
\end{table}

\emph{Dimensions 28, 29 and 30: one layer.} Take $h=\sqrt2$ and the head from
a lattice vector of squared length $8$: $x=v/2$, so that $\ip{x}{u}\le2$ with
equality and the layer deletes nothing. Three of its four constraints are then
free \emph{in every dimension}. The third line of \eqref{eq:heights} reads
$\ip{w}{w'}\le0$, so one head carries a whole cross-polytope, $2k$ directions.
The fourth reads $\ip{x}{x'}\le0$, so two heads share a direction as soon as
their lattice vectors are non-acute --- and the $48$ vectors
$\pm f_{1},\dots,\pm f_{24}$ of a frame satisfy that throughout, while $48$
vectors of $\R^{24}$ pairwise at non-positive inner product \emph{are} a frame,
so $24$ head lines is the most there is. The layer is
\begin{equation}\label{eq:framelayer}
  48\times2k \;=\; 96k \quad\text{points},
\end{equation}
$384$, $480$, $576$, $672$ in dimensions $28$, $29$, $30$, $31$.

What is left is \eqref{eq:layercap}, and it is the whole content. If
every owner is \emph{type $B$} for the frame --- $|\ip{u}{f_{i}}|=2$ for all
$24$ heads, never $3$ or $4$ --- the threshold on the directions is
\begin{equation}\label{eq:framethr}
  \ip{z}{w}\;\le\;\sqrt{3/2}-\tfrac12\;=\;0.7247448\ldots,
\end{equation}
and the $\tau(k)$ cap directions are a root system: $D_{4}$, $D_{5}$, $E_{6}$,
$E_{7}$ each have an orthonormal frame in which \emph{every} root has largest
coordinate $1/\sqrt2=0.7071067\ldots$ For $E_{7}$ one has to rotate a pair of
basis vectors by $45^{\circ}$, since two of its roots are otherwise aligned with
a frame vector. So the margin is a single inequality, and it is the one the
norm of the head buys:
\begin{equation}\label{eq:sqrt2}
  1/\sqrt2\le\sqrt{3/2}-\tfrac12
  \quad\Longleftrightarrow\quad 1+1/\sqrt2\le\sqrt3
  \quad\Longleftrightarrow\quad \sqrt2<\tfrac32 .
\end{equation}
A norm-$6$ head at the same height would need $\ip{f}{u}\le1$ instead, which
forbids $11\,730$ minimal lines to each head where the norm-$8$ head forbids the
$2\,094$ at $|\ip{v}{u}|\ge3$.

So the layer costs nothing geometrically and everything in the classes: every
one of them must be type $B$ for one common frame --- $8$ classes in dimension
$28$, $24$ in dimension $30$, $42$ in dimension $31$, and $14$ in dimension
$29$, which carries twelve zero-sum triangles and two antipodal pairs because
$\tau(5)=40$ is not a multiple of $3$. That is cheap for one reason. Choose the coordinates so that the frame is $8e_{1},\dots,8e_{24}$;
then the type-$B$ lines are exactly the $759\times64=48\,576$ octad vectors
$(\pm2^{8},0^{16})$ of the Golay code, and the whole monomial group
$2^{12}{:}M_{24}$ permutes them. \emph{Every monomial image of a type-$B$ class
is again a type-$B$ class}, so nothing has to be filtered and what is left is a
packing. It is a packing with cheap moves as well: a type-$B$ line is an octad
together with a position in $\mathbb{F}_{2}^{6}$, a Golay sign word acts on it by
translating that position inside its octad, and the sign word of a class is
therefore an exactly solvable coordinate. Coordinate descent over the sign
words, with a fresh batch of permutations at each visit to a class, gives $14$
pairwise disjoint $248$-line classes in dimension $29$ --- the full
$14\cdot248$, with nothing given up. Dimension $30$ is exact as well, at
$24\cdot248=5\,952$, once the descent is replaced by freezing a core of twelve
disjoint classes, pooling the monomial images disjoint from all of it and
solving maximum clique on that pool. A subset of a class is again a class, so
where a family is not exact the repeats are simply dropped.

What the layer costs in axis points is second order, and a rotation recovers
part of it. The condition is $|\ip{a}{w}|\le1/\sqrt2$, and what fails it is an
axis point lying along a frame direction: of the axis of \cite{ma-etal2025},
carried into the frame's coordinates, $16$ of $24$ survive in dimension $28$ and
$32$ of $40$ in dimension $29$, where the eight that die sit at
$|\ip{a}{w}|=1$ exactly. A maximum $60^{\circ}$ code is a rotated copy of the
root system and the rotation is free, so in dimensions $30$ and $31$ the axis is
turned before it is carried across --- by $45^{\circ}$, $45^{\circ}$ and
$90^{\circ}$ in three coordinate planes, which keeps every coordinate an integer
quadruple $(a+b\sqrt2+c\sqrt3+d\sqrt6)/24$ --- and $68$ of $72$ and $118$ of
$126$ survive, against $48$ and $110$ unrotated. Hence
\[
  192\,592+3\cdot3\,968+16+384=\Kh{28},
\]
\[
  189\,616+19\,840+32+480=\Kh{29},\quad
  184\,656+35\,712+68+576=\Kh{30} ,
\]
the middle count having $12$ zero-sum triangles and $2$ antipodal pairs, since
$\tau(5)=40$ is not a multiple of $3$ and the $40$ directions of a maximum
$60^{\circ}$ code in $\R^{5}$ sum to zero, so $13$ disjoint triangles cannot be
carved out of one.

\emph{Dimension 31: the same layer, and a density.} Here $96k=672$, and the
layer pays as soon as the $42$ type-$B$ classes hold at least $10\,251$ of the
$10\,416$ lines that $42$ full classes give --- $98.4$ per cent. The descent
above reaches $10\,183$, and for a while that looked like the end of it. It is
not an existence question: two uniform monomial images of the $248$-line class
are disjoint with probability $p=0.4319$, so a family of $42$ is a clique in a
graph of that density on the $\sim\!10^{12}$ images, where the random-graph
clique number $2\ln(10^{12})/\ln(1/p)$ is about $66$. It is a question about
what a search that can only \emph{sample} can reach: it places the $k$-th class
while $np^{k-1}\ge1$ and therefore stops at $k=1+12\ln10/\ln(1/p)$, which is
$34$ at $p=0.4319$, so forty-two needs $p\ge10^{-12/41}=0.5097$. The density is
a design choice. Writing $S_{i}$ for the set of $44$ octads supporting class
$i$, the probability that two images are disjoint falls with $|S_{i}\cap S_{j}|$,
and
\[
  \sum_{i<j}|S_{i}\cap S_{j}|=\sum_{O}\binom{n_{O}}{2},\qquad
  \sum_{O}n_{O}=42\cdot44=1\,848
\]
over the $759$ octads, so a flat octad coverage is worth more than any amount of
further sampling: the sum is $1\,898$ for the family the uniform scan produced
and at least $1\,419$ for any $42$ supports at all. Screening candidates on the
free slots their octads still have, rather than on what they cost now, gives a
family at $1\,670$ with $p=0.5467$ and $10\,328$ of the $10\,416$ owner lines,
and
\[
  175\,904+61\,968+118+672=\Kh{31} .
\]

How far this can go is known to different depths. In all four dimensions the
layer is at the ceiling \eqref{eq:framelayer} --- every vector of a frame on
every direction of a cross-polytope, and $24$ head lines is the most $\R^{24}$
holds. So is the axis, and exhaustively: a further axis point would be a
\emph{unit} vector of $\{a:\ip{a}{a_{i}}\le\tfrac12,\ \ip{a}{z}\le\sqrt3/2,\
|\ip{a}{w}|\le1/\sqrt2\}$, whose right-hand sides are all positive, so that set
is a polytope with the origin interior and the convex $|a|$ attains its maximum
at a vertex; enumerating the vertices of all four gives $0.7654$, $0.7929$,
$0.8165$ and $0.8660$, and none of the four axes admits another point at all.
The direction weight $w=\sum_{i}(|Z_{i}|-1)$ is at $\lfloor2\tau(k)/3\rfloor$,
which is a theorem: two classes sharing a direction would meet at
$\ip{u}{u'}\le1$ throughout and so be one class, so the parts are disjoint; a
part is pairwise at $\le-\tfrac12$ and $|\sum z|^{2}\ge0$ holds it to three;
across parts the cosines are $\le\tfrac12$; hence $\bigcup Z_{i}$ is a
$60^{\circ}$ code, $3t+2q\le\tau(k)$, and the maximum of $2t+q$ over that is
$\lfloor2\tau(k)/3\rfloor$, attained here as $16$, $26$, $48$ and $84$. Only
$\tau(4)=24$ is proved, so only dimension $28$ sits at an unconditional ceiling:
against the proved $\tau(5)\le44$, $\tau(6)\le77$ and $\tau(7)\le134$ the
unconditional weights are $29$, $51$ and $89$, worth $1\,488$, $1\,488$ and
$2\,480$ points to anyone who improves a low-dimensional kissing number. What is
otherwise left is the class problem of \S\ref{ss:class}, at $+32$, $+52$, $+96$
and $+168$ for every extra line of the $248$-line independent set, against the
Delsarte ceiling of $425.45$ lines; and in dimension $31$ also the packing,
where $88$ repeated lines are worth up to $+352$.

\begin{sloppypar}
Three packages verify the four --- \texttt{dim28-norm8-frame-layer},
\texttt{dim29-30-frame-layer} and \texttt{dim31-frame-layer}. Each rebuilds the
$196\,560$ minimal vectors from the Golay code and checks the inner-product
signature of the $98\,280$ lines before trusting them, carries every direction
and axis point as an exact algebraic number --- an integer quadruple
$(a+b\sqrt2+c\sqrt3+d\sqrt6)/24$ for the frame layer --- and decides every pair
exactly: integer inner products in $\R^{24}$, and $\Q(\sqrt2,\sqrt3)$ in
$\R^{k}$. Each
checks that the classes are disjoint sets of minimal vectors with pairwise inner
products at most $1$, that the heads are lattice vectors of the stated squared
length, that no equator point is deleted, and that the four families have
pairwise distinct $(|x|^{2},|y|^{2})$, so that the count adds no point twice.
\end{sloppypar}
...
worth most when it is a triple: the number of triples is at most
$\lfloor K(k)/3\rfloor$, which is $0,2,4,8,13,24,42$, so at most $42\times248$
lines carry caps; one therefore takes the independent sets as large as possible.
At $k=5$ the bound $13$ is not attained --- the $40$ directions of a maximum
$60^{\circ}$ code in $\R^{5}$ sum to zero, so thirteen disjoint zero-sum triples
would leave a fortieth direction summing to zero on its own --- but the weight
$\sum_{i}(|Z_{i}|-1)=26$ is, by twelve triples and two antipodal pairs. At larger
...
$1\,932$ of them are poles and the total is $589\,680+1\,932=\Kh{38}$, which is
...
go from $2\,340$, $5\,338$ and $7\,374$ to $2\,562$, $5\,704$ and $7\,624$ poles, against the published records $2\,564$, $5\,730$ and $\Kp{18}$.
At $k=18$ that budget is not the best this paper knows: \S\ref{sec:d18} gives $\Kh{18}$ points in dimension $18$, some $700$ more than the poles placed here. They are not used, and whether they can be is open. The configurations carried in above are read from \cite{cohn-data} as integers against an integer metric, whereas the one of \S\ref{sec:d18} has its $\R^{2}$ part at multiples of $30^{\circ}$ and so is defined over $\Q(\sqrt3)$, not over $\Q$ in an $18$-dimensional rational space. Dimension $83$ below places a configuration realised over $\Z[\sqrt2]$, but there the directions and the poles lie in one ring; here they would not.
...
\[
  327\,680+128\cdot3\,324+4\cdot741 \;=\; 327\,680+425\,472+2\,964 \;=\; \Kh{39} . \qedhere
\]
\end{proof}
...
$K(62)\ge\Kh{62}$ and $K(63)\ge\Kh{63}$.
\end{theorem}
...
\section{Dimension 18: a hexagon of bent cosets}\label{sec:d18}

The one improvement below dimension $25$ uses none of the three extremal lattices
above, and no code chain either.
Write a point of $\R^{18}=\R^{16}\oplus\R^{2}$ as $(v,p)$ in units where the norm
is $8$, so that two points are compatible exactly when $\ip{(v,p)}{(v',p')}\le4$.
The configuration is a hexagon:

\begin{table}[ht]
\caption{The dimension-$18$ configuration. The first three rows are the layout of
\cite{cohn-li2024}; the whole improvement is in the fourth.}
\label{tab:d18}
\small
\begin{tabular}{llll r}
\toprule
 & $|p|^{2}$ & angles of $p$ & $v$ & points\\
\midrule
equator & $0$ & --- & odd $\mathrm{BW}_{16}$ & $480+30\cdot128=4\,320$\\
tiers & $2$ & $0^\circ,60^\circ,\dots,300^\circ$ & $\pm1$ on a $6$-set of that angle's family & $6\cdot16\cdot32=3\,072$\\
poles & $8$ & $0^\circ,60^\circ,\dots,300^\circ$ & $0$ & $6$\\
tier B & $8/9$ & $30^\circ,90^\circ,\dots,330^\circ$ & $(2/3)(-1)^{c}$, $c$ a sign word & $6\cdot160=960$\\
\midrule
total & & & & $\Kh{18}$\\
\bottomrule
\end{tabular}
...
Here odd $\mathrm{BW}_{16}$ means the $480$ vectors $(\pm2,\pm2,0^{14})$ together
with the vectors supported on an octad of $\mathrm{RM}(1,4)$ with entries $\pm1$
and an odd number of minus signs. Decoding the $\Kp{18}$ of \cite{cohn-li2024}
exhibits exactly this hexagon, with their outer tier occupying the two angles
$0^\circ$ and $180^\circ$ and carrying $256$ vectors. The gain of $\Kgain{18}$ is
entirely in that outer tier, and it comes from choosing the three six-set
families differently.

\subsection{What the outer tier costs}

Let $c\in\mathbb F_2^{16}$ be the sign word of a tier-B vector. Against the odd
octad vectors $c$ must be even on every octad, that is $c\in\mathrm{RM}(2,4)$;
against a tier at angular distance less than $90^\circ$ it must be even on every
six-set of that tier's family; at $90^\circ$ or more there is no constraint at
all, which is why the six angles $30^\circ+60^\circ k$ are better than the two
that \cite{cohn-li2024} use. For $c\in\mathrm{RM}(2,4)$ the parity
$\ip{c}{\mathbf 1_X}$ depends only on the coset of $\mathrm{RM}(1,4)$ containing
$\mathbf 1_X$. A family whose sixteen six-sets all lie in \emph{one} coset
$z+\mathrm{RM}(1,4)$ therefore costs tier B a single linear condition
$\ip{c}{z}=0$, whereas a family spread over two cosets costs two.

The cosets of $\mathrm{RM}(1,4)$ inside $\mathrm{RM}(2,4)$ that contain weight-$6$
words are exactly the $28$ \emph{bent} cosets, of weights $6$ and $10$, sixteen
words each. The sixteen weight-$6$ words of a bent coset form a symmetric
$2$-$(16,6,2)$ design, so they pairwise meet in two cells; that is a valid tier,
and a maximal one --- sixteen is the largest family of admissible six-sets
\cite{repo}.

\subsection{Which hexagons are open}

On $\mathrm{RM}(2,4)/\mathrm{RM}(1,4)\cong\mathbb F_2^{6}$ the pairing
$B(q,z)=\ip{q}{z}$ is a nondegenerate symplectic form, and ``bent'' is the
quadric $O^{+}(6,2)$ of $28$ nonsingular points. For bent $q$ and $z$ one has
$B(q,z)=1$ if and only if $q+z$ is bent --- and that is also exactly the condition
for two tiers $60^\circ$ apart to be compatible, their six-sets meeting in at
most three cells. A hexagon of tiers is therefore a triangle $z_0,z_1,z_2$ in the
bent-sum graph, and there are two kinds.

If $z_2=z_0+z_1$, the triangle is \emph{closed}: all twelve possible tier-B
angles see the same $512$-word space, and since a word may be used at only one
angle the outer tier is capped at $512$.

If $z_0+z_1+z_2\notin\mathrm{RM}(1,4)$ --- \emph{open}, which $280$ of the $336$
triangles are --- the six angles $30^\circ+60^\circ k$ see three \emph{different}
$512$-word spaces, pairwise meeting in a $256$-word one. Each is an affine
$4$-space of $\mathrm{RM}(1,4)$-cosets over an elliptic quadric, and full cosets
pairwise at bent difference form a distance-$6$ code: the bent-difference graph on
those sixteen cosets is the complement of the Clebsch graph, whose clique number
is $5$, so each angle carries $5\cdot32=160$ words. The six sets of $160$ are
pairwise disjoint, since distinct words of $\mathrm{RM}(2,4)$ are automatically at
distance at least $4$, which is all that angles $60^\circ$ or more apart require.
This gives $6\cdot160=960$ and
\[
  K(18)\;\ge\;4\,320+3\,072+6+960\;=\;\Kh{18}.
\]

The first family of \cite{cohn-li2024} is one of these $28$ bent cosets; their
other two are each half of one bent coset and half of another. That costs an extra
linear condition apiece and makes every angle see the same $256$ words, so $256$
was indeed a ceiling --- for their base, with the code varying, rather than for the
construction. Dimension $18$ is discussed again in \S\ref{ss:obstructions}, where
the ceiling that does hold is the one for the \emph{other} Cohn--Li shape.

\section{The results}\label{sec:results}

\begin{longtable}{rrrl}
\caption{All fifty-three improvements. ``Previously'' is \cite{cohn-table} where
...
\bottomrule
\endfoot
$18$ & $\Kp{18}$ & $\Kh{18}$ & bent-coset hexagon over odd $\mathrm{BW}_{16}$ (\S\ref{sec:d18})\\
$25$ & $\Kp{25}$ & $\Kh{25}$ & layers over $\Leech$, lens heads (\S\ref{ss:d25})\\
$26$ & $\Kp{26}$ & $\Kh{26}$ & layers over $\Leech$, one hexagon of leans (\S\ref{ss:d2627})\\
$27$ & $\Kp{27}$ & $\Kh{27}$ & layers over $\Leech$, two coset triangles (\S\ref{ss:d2627})\\
$28$ & $\Kp{28}$ & $\Kh{28}$ & second layer over $\Leech$, norm-$8$ head (\S\ref{ss:d293031})\\
$29$ & $\Kp{29}$ & $\Kh{29}$ & second layer over $\Leech$, norm-$8$ frame (\S\ref{ss:d293031})\\
$30$ & $\Kp{30}$ & $\Kh{30}$ & second layer over $\Leech$, norm-$8$ frame (\S\ref{ss:d293031})\\
$31$ & $\Kp{31}$ & $\Kh{31}$ & second layer over $\Leech$, norm-$8$ frame (\S\ref{ss:d293031})\\
$38$ & $\Kp{38}$ & $\Kh{38}$ & layers over $\Leech$, $k=14$\\
$39$ & $\Kp{39}$ & $\Kh{39}$ & constant-weight codes\\
$49$ & $\Kp{49}$ & $\Kh{49}$ & layers over $\Pff$\\
$50$ & $\Kp{50}$ & $\Kh{50}$ & layers over $\Pff$\\
$51$ & $\Kp{51}$ & $\Kh{51}$ & layers over $\Pff$\\
$52$ & $\Kp{52}$ & $\Kh{52}$ & layers over $\Pff$\\
$53$ & $\Kp{53}$ & $\Kh{53}$ & layers over $\Pff$\\
$54$ & $\Kp{54}$ & $\Kh{54}$ & layers over $\Pff$\\
$55$ & $\Kp{55}$ & $\Kh{55}$ & layers over $\Pff$\\
$56$ & $\Kp{56}$ & $\Kh{56}$ & layers over $\Pff$\\
$57$ & $\Kp{57}$ & $\Kh{57}$ & layers over $\Pff$\\
$58$ & $\Kp{58}$ & $\Kh{58}$ & layers over $\Pff$\\
$59$ & $\Kp{59}$ & $\Kh{59}$ & layers over $\Pff$\\
$60$ & $\Kp{60}$ & $\Kh{60}$ & layers over $\Pff$\\
$61$ & $\Kp{61}$ & $\Kh{61}$ & layers over $\Pff$\\
$62$ & $\Kp{62}$ & $\Kh{62}$ & \textsc{ers} chain $(62,15,2)$\\
$63$ & $\Kp{63}$ & $\Kh{63}$ & \textsc{ers} chain $(63,15,2)$\\
$68$ & $\Kp{68}$ & $\Kh{68}$ & cross-section of $\Gsz$, $k=4$\\
$69$ & $\Kp{69}$ & $\Kh{69}$ & cross-section of $\Gsz$, $k=3$\\
$70$ & $\Kp{70}$ & $\Kh{70}$ & cross-section of $\Gsz$, $k=2$\\
$71$ & $\Kp{71}$ & $\Kh{71}$ & cross-section of $\Gsz$, $k=1$\\
$73$ & $\Kp{73}$ & $\Kh{73}$ & layers over $\Gsz$\\
$74$ & $\Kp{74}$ & $\Kh{74}$ & layers over $\Gsz$\\
$75$ & $\Kp{75}$ & $\Kh{75}$ & layers over $\Gsz$\\
$76$ & $\Kp{76}$ & $\Kh{76}$ & layers over $\Gsz$\\
$77$ & $\Kp{77}$ & $\Kh{77}$ & layers over $\Gsz$\\
$78$ & $\Kp{78}$ & $\Kh{78}$ & layers over $\Gsz$\\
$79$ & $\Kp{79}$ & $\Kh{79}$ & layers over $\Gsz$\\
$80$ & $\Kp{80}$ & $\Kh{80}$ & layers over $\Gsz$\\
$81$ & $\Kp{81}$ & $\Kh{81}$ & layers over $\Gsz$\\
$82$ & $\Kp{82}$ & $\Kh{82}$ & layers over $\Gsz$\\
$83$ & $\Kp{83}$ & $\Kh{83}$ & layers over $\Gsz$\\
$84$ & $\Kp{84}$ & $\Kh{84}$ & layers over $\Gsz$\\
$85$ & $\Kp{85}$ & $\Kh{85}$ & layers over $\Gsz$\\
$86$ & $\Kp{86}$ & $\Kh{86}$ & layers over $\Gsz$\\
$87$ & $\Kp{87}$ & $\Kh{87}$ & layers over $\Gsz$\\
$88$ & $\Kp{88}$ & $\Kh{88}$ & layers over $\Gsz$\\
$89$ & $\Kp{89}$ & $\Kh{89}$ & layers over $\Gsz$\\
$90$ & $\Kp{90}$ & $\Kh{90}$ & layers over $\Gsz$\\
$91$ & $\Kp{91}$ & $\Kh{91}$ & layers over $\Gsz$\\
$92$ & $\Kp{92}$ & $\Kh{92}$ & layers over $\Gsz$\\
$93$ & $\Kp{93}$ & $\Kh{93}$ & layers over $\Gsz$\\
$94$ & $\Kp{94}$ & $\Kh{94}$ & layers over $\Gsz$\\
$95$ & $\Kp{95}$ & $\Kh{95}$ & layers over $\Gsz$\\
$96$ & $\Kp{96}$ & $\Kh{96}$ & \textsc{ers} chain $(96,24,6,1)$\\
\end{longtable}
...
$6\,500+768=7\,268$, short of the $\Kp{18}$ that \cite{cohn-li2024} prove in that
dimension by a configuration of a different shape. That other shape is the hexagon
of \S\ref{sec:d18}, and it is \emph{not} at a ceiling: its outer tier rises from
$256$ to $960$, which is the whole of the improvement recorded there. A ceiling
established for one shape says nothing about the dimension while another shape
holds its record.
...
\emph{The dimension-$25$ lens-head construction is bounded by $197\,623$.}
Its equator is fixed, and every head removes exactly one equator point. Two
heads can never share a removal: a head $x$ that removes the equator point $p$
has $\ip{x}{p}\ge(18+\sqrt{30})/7=3.35389\ldots$, which one spherical $7$-design
proves, whereas two compatible heads sharing a removal would both need
$\ip{x}{p}\le\tfrac{10}{3}$; over the class heads of the shipped configuration
the smallest such inner product is $3.36603$. The count $196\,560+H+3$ of \S\ref{ss:d25} is
therefore exact, and only $H$ is at issue. The
class-head core is a union of blocks, and over the entire norm-$6$ shell of
$16\,773\,120$ vectors it is capped at $971$ heads. The supplement is an
independent set in one explicit graph, on the complete pool of $1\,188$
candidate heads over $1\,188$ distinct owners, whose density is $0.1538$;
constraint programming bounds its independence number by $89$. With the two
poles and the one non-lattice equator point contributing $3$, the construction
cannot exceed $196\,560+971+89+3=197\,623$. The shipped configuration is within
$44$ of that, so a further gain in dimension $25$ requires a head outside the
block-plus-supplement family altogether. The instance does not close, so $89$ is
what a particular solve proves rather than the independence number itself; a fixed
seed reproduces it, and other runs of the same model have proved bounds as low as
$88$. Every such bound is valid, so the figure is a ceiling on the construction in
any case.

\emph{The second-layer trades in dimensions $25$ to $27$ fail at opposite ends
of one structure.} Dimensions $26$ and $27$ carry a second cap layer above the
first, whose heads have $|x|^{2}=3$, sit at height $1$, carry one antipodal pair
of directions and are worth $+1$ apiece against a first-layer head's $+2$. A
candidate is admitted only when \emph{every} first-layer head blocking it is
deleted, so what governs the trade is not how often blockers are shared on
average but whether candidates with few blockers share those blockers with each
other. Neither is it enough to price one trade at a time: the class heads, the
free heads and the second layer compete for the same owners, and a trade that
loses in isolation can pay once the other two are allowed to move with it, which
is how the configuration of \S\ref{ss:d2627} was found. Dimension $27$ fails although the sharing is high. Its next admissible
tier has $10\,030$ candidates carrying $252\,582$ incidences on $2\,095$ blocking
heads, some $121$ candidates apiece, yet each candidate carries about $25$
blockers and needs all of them at once: deleting the $1\,200$ heads that block
the most unlocks $253$ candidates, and the trade closes only when the whole class
layer is gone, at a cost of $4\,190$.

Dimension $25$ inverts the exchange rate and still gains nothing. Its lens heads
are forced to height $1$, since the poles require $y\le1$ and a mirror pair
$y\ge1$; surrendering the two poles opens a second layer at $y=2/\sqrt3$,
$|x|^{2}=8/3$, $x=\tfrac23v$ with $v\in\Leech$ of norm $6$. For such a head the
condition $\ip{x}{z}\le2$ against the equator reads $\ip{v}{z}\le3$, and every
norm-$6$ vector attains exactly $3$, so the head removes nothing at all: it is
worth $+2$ where a lens head, which pays for itself with a removal, is worth
$+1$. Deleting a lens head therefore costs $1$ and admitting a free head gains
$2$, the opposite of the ratio in dimensions $26$ and $27$. The trade is
nevertheless empty. Of $1.3$ million norm-$6$ directions sampled against the
shipped layer, the $654$ blocked by at most ten lens heads are blocked by $630$
distinct heads between them, barely one apiece. Here the blockers are
cheap but private, and maximising $2|F|-|D|-2$ over that instance is optimal at
$0$. Neither the exchange rate nor the average sharing settles such a trade; what
settles it is whether a set of candidates can be found whose blockers coincide.

Nor does dimension $25$ admit the other kind of second layer, the one that carries
dimensions $26$ and $27$: class heads at $|x|^{2}=8/3$, of the form $x=u+v/3$ with
$u$ minimal and $|v|^{2}=6$, $\ip{u}{v}=-3$. At $k=1$ such a head carries both
directions $\pm1$, since two directions of one head need
$\ip{w}{w'}\le1-2/h^{2}=-\tfrac12$ and $\R^{1}$ offers only $-1$; it therefore
gives two cap points for the one equator point it removes, worth $+1$, exactly as
a lens head is, and it costs the two poles. Three of them would already improve on
$\Kh{25}$. None exists. Such a head must meet every lens head at
$\ip{x}{x'}\le2-2/\sqrt3$, a cosine of $0.2989$, and over $5.1$ million of them,
drawn from the blocks of as many norm-$6$ vectors, the smallest cosine attained
against the shipped layer is $0.4034$.
...
or an argument using the lattice structure. B.~Lindow has since obtained $418$
for lines of minimal vectors of $\Leech$, by a three-point bound over the
Terwilliger algebra of the automorphism group of $\Leech$ with an exact rational
dual (private communication, September 2026); for an arbitrary line system in
$\R^{24}$ at $|\cos|\le\tfrac14$ the bound $425$ stands.
...
\item \emph{Configurations small enough to write down are checked in
      ...
      $\binom{200540}{2}=20\,108\,045\,530$ pairs; that configuration is now
      superseded (\S\ref{ss:d2627}), and the two configurations of \S\ref{ss:d2627},
      in dimensions $26$ and $27$, are verified in exact arithmetic, the
      head--axis inequalities in $\mathbb{Q}(\sqrt2,\sqrt3)$. The four of
      \S\ref{ss:d293031} are verified the same way, in
      $\mathbb{Q}(\sqrt2,\sqrt3)$, and each of those checks includes that
      the four families of points have pairwise distinct
      $(|x|^{2},|y|^{2})$ --- a statement about the point set rather than about
      any angle, and the one the count rests on.
...
same model; those of \S\ref{ss:d25} and \S\ref{ss:d2627} with Claude Fable~5.1
in the same way, and those of \S\ref{ss:d293031} with Claude Code again, all
three inside the joint work with H.~Cohn and B.~Lindow. The agent proposed constructions, implemented and ran the verification
