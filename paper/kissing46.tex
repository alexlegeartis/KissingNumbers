...
gives $K(28)\ge204\,896$, $K(29)\ge209\,968$, $K(30)\ge220\,948$ and
$K(31)\ge238\,354$, so that every dimension from $25$ to $31$ is improved.
...
second layers over $\Leech$ & $28$--$31$ & $4$ & $+508$ & $1.00$--$1.00$ & \S\ref{ss:d293031}\\
...
$K(28)\ge204\,896$, $K(29)\ge209\,968$, $K(30)\ge220\,948$ and
$K(31)\ge238\,354$.
\end{theorem}
...
four that occur. The norm of the head is a second free parameter, and the two
entries at $h^{2}=2$ differ only in it. One consequence of \eqref{eq:layercap} is worth
...
\begin{table}[ht]
\caption{The four layers of Theorem~\ref{thm:293031}, in the normalisation
$|P|^{2}=4$. The head $x$ is the $\R^{24}$ part of a layer point and $w$ its
direction; $z$ is a cap direction. The first three columns are one layer, the
norm-$8$ frame at height $\sqrt2$: the head runs over all $48$ vectors of a
Leech frame and the direction over the whole cross-polytope of $\R^{k}$, which
is $96k$ points. Dimension $31$ is the one place it does not pay, and keeps the
height-$\sqrt3$ layer. The axis is the axis of \cite{ma-etal2025} that
Table~\ref{tab:calib} reproduces, carried into the frame's coordinates. No layer
point deletes an equator point, and the last three lines are the whole price.}
\label{tab:layers}
...
\midrule
height $h$ & $\sqrt2$ & $\sqrt2$ & $\sqrt2$ & $\sqrt3$\\
$|x|^{2}=4-h^{2}$ & $2$ & $2$ & $2$ & $1$\\
$|$head$|^{2}$ & $8$ & $8$ & $8$ & $4$\\
head $x$ & $v/2$ & $v/2$ & $v/2$ & $u_{0}/2$\\
directions $w$ & $\pm e_{1..4}$ & $\pm e_{1..5}$ & $\pm e_{1..6}$ & $56$ holes of $E_{7}$\\
axis threshold $1/h$ & $1/\sqrt2$ & $1/\sqrt2$ & $1/\sqrt2$ & $1/\sqrt3$\\
largest $\ip{w}{z}$ & $1/\sqrt2$ & $1/\sqrt2$ & $1/\sqrt2$ & $1/\sqrt3$\\
directions per head & $8$ & $10$ & $12$ & $4$\\
heads per direction & $48$ & $48$ & $48$ & $2$\\
layer points & $384$ & $480$ & $576$ & $8$\\
\midrule
axis points kept & $16$ of $24$ & $32$ of $40$ & $48$ of $72$ & $122$ of $126$\\
owner lines & $8\cdot248$ & $14\cdot248$ & $5\,941$ & $42\cdot248$\\
net gain & $+376$ & $+472$ & $+508$ & $+4$\\
\bottomrule
\end{tabular}
\end{table}
...
\end{equation}
$384$, $480$, $576$ in dimensions $28$, $29$, $30$.
...
$28$, $24$ in dimension $30$, and $14$ in dimension $29$, which carries twelve
zero-sum triangles and two antipodal pairs because $\tau(5)=40$ is not a
multiple of $3$. That is cheap for one reason. Choose the coordinates so that the frame is $8e_{1},\dots,8e_{24}$;
...
$14\cdot248$, with nothing given up --- and $5\,941$ of the $5\,952$ owner
lines in dimension $30$. A subset of a class is again a class, so
the repeats are simply dropped.

What the layer costs in axis points is second order. The condition is
$|\ip{a}{w}|\le1/\sqrt2$, and what fails it is exactly an axis point lying along
a frame direction: $16$ of $24$ survive in dimension $28$, $32$ of $40$ in
dimension $29$, $48$ of $72$ in dimension $30$. Hence
\[
  192\,592+3\cdot3\,968+16+384=204\,896,
\]
\[
  189\,616+19\,840+32+480=209\,968,\quad
  184\,678+35\,646+48+576=220\,948 ,
\]
...
\emph{Dimension 31.} Here $96k=672$ would need $42$ type-$B$ classes holding at
least $10\,254$ of the $10\,416$ lines that $42$ full classes give, and the
descent reaches $10\,183$; the layer of \eqref{eq:framelayer} does not pay
there, and dimension $31$ keeps a different one. Take $h=\sqrt3$ and
$x=u_{0}/2$ for a minimal vector $u_{0}$ that is not an owner. The axis
threshold is $1/\sqrt3$, and the covering radius of the $E_{7}$ root system on
$S^{6}$ is exactly $\arccos(1/\sqrt3)$, attained at the $56$ minimal directions
of $E_{7}^{*}$; take $w$ to be one of them. A deep hole meets the cap directions
at $0$ or $\pm1/\sqrt3$, so \eqref{eq:capthr} applies, and
$\ip{x}{u}=\ip{u_{0}}{u}/2\le1$ because $u_{0}$ is not an owner: the layer meets
no cap, whatever the classes are. Alone among the four it does not touch the
class problem, and the classes of \cite{ma-etal2025} are used unchanged. The
worst value in \eqref{eq:layercap} is $\sqrt{2/3}+2/\sqrt3=1.9712\ldots$, with
exact margin $\sqrt6+2\sqrt3<6$. The third line of \eqref{eq:heights} reads
$\ip{w}{w'}\le\tfrac13$, which two antipodal pairs of deep holes at mutual
cosine $\pm\tfrac13$ satisfy, and the fourth reads $\ip{x}{x'}\le-1$, which
admits the two signs $\pm u_{0}/2$ and no more: $2\times4=8$ points. Each
deep-hole line deletes the axis points within $\arccos(1/\sqrt3)$ of it, and two
lines chosen as above delete four between them, so
\[
  175\,728+3\cdot20\,832+(126-4)+8=238\,354 .
\]

How far each can go is known to different depths. In dimensions $28$, $29$ and
$30$ the layer is at the ceiling \eqref{eq:framelayer} --- every vector of a
frame on every direction of a cross-polytope, and $24$ head lines is the most
$\R^{24}$ holds --- and the direction weight $\sum_{i}(|Z_{i}|-1)$ is at the
$60^{\circ}$-code bound in $\R^{k}$. What is left is the class problem of
\S\ref{ss:class}, at $+32$, $+52$ and $+96$ for every extra line of the
$248$-line independent set, against the Delsarte ceiling of $425.45$ lines; and
in dimension $30$ also the packing, where $11$ repeated lines are worth $+44$.
In dimension $31$ the height-$\sqrt3$ layer is at a ceiling for that rotation of
the axis: for two distinct deep-hole lines $w_{i}\mp w_{j}$ is a root of the cap
$E_{7}$, so a full $126$-point axis hosts at most one line that deletes nothing,
and a search over every subset of that axis which may be deleted returns $4$ as
the optimum. A different rotation would change which axis points a hole blocks,
and that search is open --- as is the $71$-line gap that would hand dimension
$31$ the frame layer as well.

\begin{sloppypar}
Three packages verify the four --- \texttt{dim28-norm8-frame-layer},
\texttt{dim29-30-frame-layer} and \texttt{dim31-sqrt3-layer}. Each rebuilds the
...
$30$ & $220\,440$ & $220\,948$ & second layer over $\Leech$, norm-$8$ frame (\S\ref{ss:d293031})\\
$31$ & $238\,350$ & $238\,354$ & second layer over $\Leech$, height $\sqrt3$ (\S\ref{ss:d293031})\\
